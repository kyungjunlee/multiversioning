...
void time_workload_creation(ExperimentConfig exp_conf) {
  std::chrono::system_clock::time_point time_start, time_end;
  std::vector<double> results;
  for (unsigned int i = 0; i < 10; i++) {
    time_start = std::chrono::system_clock::now();
    barrier();
    auto workload = ActionFactory<RMWBatchAction>::generate_actions(
  ...
  for (unsigned int i = 0; i < 10; i++) {
    std::cout << i << "\t\t" << 
      exp_conf.num_txns << "\t\t" << 
      results[i] << "\t\t" << 
      exp_conf.num_txns / results[i] << std::endl;
  }
};
...
int main(int argc, char** argv) {
  ...
  time_workload_creation(exp_conf);
